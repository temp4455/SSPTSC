...
from config_linear import cmd_parameter_fine_tuning,Parameter
# from model_mask_Batchnorm import TSEncoder,TSClassifcation
from model_linear import TSClassifcation,TSEncoder
from model_linear_longformer import TSClassifcation as LongformerTSClassifcation
...
if __name__=='__main__':
    ...
    # train_np[:, 1:] = (train_np[:, 1:] - train_np[:, 1:].min()) / (train_np[:, 1:].max() - train_np[:, 1:].min()) * (
    #             param.nclass - 1)
    # test_np[:, 1:] = (test_np[:, 1:] - test_np[:, 1:].min()) / (test_np[:, 1:].max() - test_np[:, 1:].min()) * (
    #             param.nclass - 1)
    ...
    #     device= torch.device('cuda:{}'.format(param.start_cuXda_id))
    # for model
    ...
    scheduler= optim.lr_scheduler.ReduceLROnPlateau(optimizer,mode='max',factor=0.5,patience=50,verbose=True,min_lr=1e-6)
